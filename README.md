...
Live demo: [https://fieldwork.ramzy.tech/](https://fieldwork.ramzy.tech/)
...
The included private Sites deployment, when available, is a local-data demonstration until Supabase variables are supplied and the app is rebuilt. An owner-private Sites login can require connectivity before the app origin opens; verify the offline PWA on its final Vercel origin without an upstream login gate.
